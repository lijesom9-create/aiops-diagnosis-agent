...
class UnifiedKnowledgeStore:
    ...
    def _merge_filters(
        ...
        """合并两个 Qdrant 风格 filter（纯逻辑在 store_utils.merge_filters，类保留委托以稳定对外契约）"""
        return merge_filters(base, extra)

    @staticmethod
    def _match_metadata_filter(
        metadata: Dict[str, Any], meta_filter: Optional[Dict[str, Any]]
    ) -> bool:
        """检查 metadata 是否满足 meta_filter（纯逻辑在 store_utils.match_metadata_filter）"""
        return match_metadata_filter(metadata, meta_filter)
    ...
    def _build_visibility_filter(
        ...
        """构造可见性 + 业务过滤条件（纯逻辑在 store_utils.build_visibility_filter）"""
        return build_visibility_filter(source, metadata_filter, org_id, user_id)
    ...
    def _build_child_filter(
        ...
        """构造子块检索 filter = chunk_type=child + 可见性过滤（纯逻辑在 store_utils.build_child_filter）"""
        return build_child_filter(source, metadata_filter, org_id, user_id)
    ...
    def _rrf_fuse(
        ...
        """加权 RRF 融合两路结果（纯逻辑在 store_utils.rrf_fuse）"""
        return rrf_fuse(results_a, results_b, k=k, weight_a=weight_a, weight_b=weight_b)

    @staticmethod
    def _tokenize(text: str) -> List[str]:
        """中英文分词（纯逻辑在 store_utils.tokenize）"""
