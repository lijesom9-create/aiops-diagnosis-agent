"""
告警 → 事故生命周期引擎（Alert Engine Service）

接收 Alertmanager 推送的告警，负责：
- 事故生命周期：新建 / 归并(escalation) / 抖动复燃 / 心跳(repeat) / 恢复闭案
- Agent 自动诊断：入队持久化诊断任务，worker 消费（初诊 / 增量重诊 / 恢复摘要）
- 成本护栏：severity 门槛、重诊硬上限、冷却、全局并发 1

本模块由 api/alerts.py 下沉而来；api/alerts.py 保留薄 handler + 端点与转发。
"""
import asyncio
import re
