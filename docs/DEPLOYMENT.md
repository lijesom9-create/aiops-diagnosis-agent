...
```
浏览器 → https://your-domain.com → Cloudflare 隧道 → VM(VM_LAN_IP) nginx
                                   │
            ┌──────────────────────┼──────────────────────┐
            │                      │                      │
...
- **公网入口**：Cloudflare 命名隧道（token 方式），路由 `your-domain.com → http://frontend:80`
- **VM**：Ubuntu 虚拟机 `VM_LAN_IP`，用户 `deploy_user`，部署目录 `/home/deploy_user/aiops-agent`
...
```
生效验证：`dig your-domain.com NS +short`（或 RDAP）。

### 2. Cloudflare 添加域名
- Cloudflare → 添加站点 `your-domain.com`（免费版）
...
push master
  ...
  │    ├─ frontend build → ghcr.io/lijesom9-create/aiops-frontend:latest
  │    ├─ admin-frontend build → ghcr.io/lijesom9-create/aiops-admin-frontend:latest
  │    └─ backend build → ghcr.io/lijesom9-create/aiops-backend:latest
...
### VM 端 runner 配置
- runner 目录：`/home/deploy_user/actions-runner/`
- systemd 服务：`actions.runner.<owner>-<repo>.agent-vm.service`（User=deploy_user，Restart=always）
- label：`self-hosted`、`agent-vm`（deploy.yml 用 `runs-on: [self-hosted, agent-vm]`）
- VM `rag-src` 是 git 仓库，origin 指向本仓库（git@github.com:<owner>/<repo>.git），用 SSH deploy key 免密拉取
...
scp rag-upd.bundle jesom@VM_LAN_IP:/home/jesom/

# 3) VM 上接收 + 推送（推送会触发 CD 自动部署）
ssh jesom@VM_LAN_IP
cd /home/deploy_user/aiops-agent
git fetch /home/jesom/rag-upd.bundle master
git reset --hard FETCH_HEAD
git push origin master      # origin 带 token，直接 push

# 4) 等 CD 完成后验证
curl https://your-domain.com/api/health/live        # {"status":"ok"}
curl -o /dev/null -w "%{http_code}" https://your-domain.com   # 200
```

### 验证 RAG 全链路（VM 内）
```bash
ssh jesom@VM_LAN_IP "bash /home/jesom/test_chat.sh"
```
...
| **deploy 时 checkout 失败** | VM clone github 超时 | deploy.yml 不用 checkout，直接 `cd /home/deploy_user/aiops-agent` 操作 |
...
- `backend/.env`（含 AI_API_KEY 等）→ 在 `.gitignore`，不会进 git；VM 上手动维护 `/home/deploy_user/aiops-agent/backend/.env`
- 持久数据卷：
...
- 公网访问：`https://your-domain.com` → 200，health ok，RAG 问答引用真实知识库
